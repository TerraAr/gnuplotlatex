...
if(inicia.arq_tabela==NULL) goto inicio;

fputs(inicio, inicia.arq_tabela);			//begin{table}
if(opcoes & FORCE_POS) fputc('!',inicia.arq_tabela);	//com [!htp] ou com [htp]
...
fputs(inicia.est_lin_horizon ? end_inicio_est : end_inicio, inicia.arq_tabela);
return inicia;
}
...
fputs(tabela.est_lin_horizon ? pos_inicio_est : pos_inicio,tabela.arq_tabela);
va_end(args);
}
...
for(uint8_t i=0U, cont_col=0U, ini=0U;*copia_format && cont_col<(*tabela).num_col;){
while(*(copia_format+i)!=' '&&*(copia_format+i)!='\t'&&*(copia_format+i)) i++;
*(copia_format+i)='\0';
...
fputs(tabela.est_lin_horizon ? comeco_final_est : comeco_final,tabela.arq_tabela);

if(tabela.caption_pos) fputs(caption,tabela.arq_tabela);
else fputs(small_sem_capt,tabela.arq_tabela);
